...
func (m *HttpListenerOptions) Hash(hasher hash.Hash64) (uint64, error) {
	...
	if h, ok := interface{}(m.GetCache()).(safe_hasher.SafeHasher); ok {
		if _, err = hasher.Write([]byte("Cache")); err != nil {
			return 0, err
		}
		if _, err = h.Hash(hasher); err != nil {
			return 0, err
		...
		if fieldValue, err := hashstructure.Hash(m.GetCache(), nil); err != nil {
			return 0, err
		} else {
			if _, err = hasher.Write([]byte("Cache")); err != nil {
				return 0, err
