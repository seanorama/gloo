...
//Source: pkg/code-generator/codegen/templates/simple_event_loop_template.go
package v1
...
type enterpriseSimpleEventLoop struct {
	emitter EnterpriseSimpleEmitter
	syncers []EnterpriseSyncer
}

func NewEnterpriseSimpleEventLoop(emitter EnterpriseSimpleEmitter, syncers ...EnterpriseSyncer) eventloop.SimpleEventLoop {
	return &enterpriseSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *enterpriseSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[EnterpriseSyncer]context.CancelFunc)
		...
		var previousSnapshot *EnterpriseSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(EnterpriseSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(EnterpriseSyncDeciderWithContext); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("enterprise.gloo.solo.io.SimpleEventLoopSync-%T", syncer))
