...
func (el *apiSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	ctx = contextutils.WithLogger(ctx, "gloosnapshot.event_loop")
	...
	go errutils.AggregateErrs(ctx, errs, emitterErrs, "gloosnapshot.emitter errors")
	go func() {
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("api.gloosnapshot.gloo.solo.io.SimpleEventLoopSync-%T", syncer))
