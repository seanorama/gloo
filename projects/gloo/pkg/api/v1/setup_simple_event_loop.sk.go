...
type setupSimpleEventLoop struct {
	emitter SetupSimpleEmitter
	syncers []SetupSyncer
}

func NewSetupSimpleEventLoop(emitter SetupSimpleEmitter, syncers ...SetupSyncer) eventloop.SimpleEventLoop {
	return &setupSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *setupSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[SetupSyncer]context.CancelFunc)
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(SetupSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(SetupSyncDeciderWithContext); isDecider {
