...
func (m *Gateway) Equal(that interface{}) bool {
	...
	case *Gateway_MatchedHttpGateways:
		if _, ok := target.GatewayType.(*Gateway_MatchedHttpGateways); !ok {
			return false
		}

		if h, ok := interface{}(m.GetMatchedHttpGateways()).(equality.Equalizer); ok {
			if !h.Equal(target.GetMatchedHttpGateways()) {
				return false
			}
		} else {
			if !proto.Equal(m.GetMatchedHttpGateways(), target.GetMatchedHttpGateways()) {
				return false
			}
		}
...
// Equal function
func (m *MatchedHttpGateway) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*MatchedHttpGateway)
	if !ok {
		that2, ok := that.(MatchedHttpGateway)
		if ok {
			target = &that2
		} else {
			return false
...
// Equal function
func (m *MatchedHttpGateways) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*MatchedHttpGateways)
	if !ok {
		that2, ok := that.(MatchedHttpGateways)
		if ok {
			target = &that2
		} else {
			return false
