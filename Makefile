...
ENVOY_GLOO_IMAGE ?= quay.io/solo-io/envoy-gloo:1.21.1-patch1
...
run-tests:
ifneq ($(RELEASE), "true")
	$(DEPSGOBIN)/ginkgo -ldflags=$(LDFLAGS) -r -failFast -trace -progress -race -compilers=4 -failOnPending -noColor --focus "uses helm to upgrade to this gloo version without errors" $(TEST_PKG)
endif
